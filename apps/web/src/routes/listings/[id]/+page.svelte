    ...
    <div>
      ...
      <div class="mt-1.5 flex flex-wrap items-center gap-2 text-sm text-gray-500">
        {#if listing.averageRating > 0}
          ...
          <span>·</span>
          <a href="/listings/{listing.id}/reviews" class="underline hover:text-gray-900">{listing.reviewCount} review{listing.reviewCount !== 1 ? 's' : ''}</a>
          <span>·</span>
        {/if}
        ...
        <section class="py-6 border-b border-gray-200">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <svg class="w-4 h-4 fill-[#ff5a5f]" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
              </svg>
              <span class="text-xl font-bold text-gray-900">{listing.averageRating.toFixed(1)}</span>
              <span class="text-gray-500">· {listing.reviewCount} review{listing.reviewCount !== 1 ? 's' : ''}</span>
            </div>
            <a
